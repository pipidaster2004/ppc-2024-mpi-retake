...
bool khokhlov_a_sum_values_by_rows_mpi::SumValByRowsMpi::ValidationImpl() {
  if (world_.rank() == 0) return (task_data->inputs_count[1] == task_data->outputs_count[0]);
  return true;
}
...
bool khokhlov_a_sum_values_by_rows_mpi::SumValByRowsMpi::RunImpl() {
  ...
  int last_row_ = row_ % world_.size();
  int local_n = (world_.rank() == world_.size() - 1) ? delta + last_row_ : delta;

  local_input_ = std::vector<int>(local_n * col_);
  std::vector<int> send_counts_(world_.size());
  std::vector<int> recv_counts_(world_.size());
  for (int i = 0; i < world_.size(); ++i) {
    send_counts_[i] = (i == world_.size() - 1) ? delta + last_row_ : delta;
    send_counts_[i] *= col_;
    recv_counts_[i] = (i == world_.size() - 1) ? delta + last_row_ : delta;
  }
  boost::mpi::scatterv(world_, input_.data(), send_counts_, local_input_.data(), 0);
  ...
  boost::mpi::gatherv(world_, local_sum.data(), local_sum.size(), sum_.data(), recv_counts_, 0);
